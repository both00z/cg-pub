...
class Vector3f {
	...
	void multiplyMatrixByVector16(float matrix[16])
	{
		float x0 = _item[0]*matrix[0]+_item[1]*matrix[4]+_item[2]*matrix[8]+_item[3]*matrix[12];
		float x1 = _item[0]*matrix[1]+_item[1]*matrix[5]+_item[2]*matrix[9]+_item[3]*matrix[13];
		float x2 = _item[0]*matrix[2]+_item[1]*matrix[6]+_item[2]*matrix[10]+_item[3]*matrix[14];
		float x3 = _item[0]*matrix[3]+_item[1]*matrix[7]+_item[2]*matrix[11]+_item[3]*matrix[15];
		_item[0] = x0; _item[1] = x1; _item[2] = x2; _item[3] = x3;
	}
...
public: 
	...
	void calculatePlanes()
	{
		...
		Vector3f v1 = _v[_trig[i]._vertex[0]];
		Vector3f v2 = _v[_trig[i]._vertex[1]];
		Vector3f v3 = _v[_trig[i]._vertex[2]];

		Vector3f plane;

		plane[0] = v1[1]*(v2[2]-v3[2]) + v2[1]*(v3[2]-v1[2]) + v3[1]*(v1[2]-v2[2]);
		plane[1] = v1[2]*(v2[0]-v3[0]) + v2[2]*(v3[0]-v1[0]) + v3[2]*(v1[0]-v2[0]);
		plane[2] = v1[0]*(v2[1]-v3[1]) + v2[0]*(v3[1]-v1[1]) + v3[0]*(v1[1]-v2[1]);
		plane[3] = -( v1[0]*( v2[1]*v3[2] - v3[1]*v2[2] ) +
	...
	void setTriangleIlluminated(int i, bool ill)
	{
		_trig[i].isIlluminated = ill;
	}

	bool isTriangleIlluminated(int i)
	{
		return _trig[i].isIlluminated;
	}

	Vector3f getPlane(int i) {return _planes[i];}

	void getTriangleVertexByIndex(int trigI, int vI, Vector3f & v)
	{
		v = _v[_trig[trigI]._vertex[vI]];
	}
